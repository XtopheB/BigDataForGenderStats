...
# knitr::include_graphics("BerlinPolice.png")
```
...
We will base our analysis on the website
<https://www.berlin.de/polizei/polizeimeldungen/archiv/>{target="_blank"} that 
...
We need specific packages for web scraping. In particular we use the package  `rvest`
...
> We begin by downloading the mainpage to have a list of available pages
...
We use functions like `html_nodes()` and `html_text()` from the *rvest* package to navigate and extract the desired data from the page. For example, to extract all *links* to individual reports, you  use:
...
reports %>%
  ...
  labs(title = "Percentages of Women-Related Reports by Year",
       subtitle = Mysubtitle,
       x = "Year", 
       y = "Percentages of reports involving women ")
